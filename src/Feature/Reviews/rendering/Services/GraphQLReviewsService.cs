...
{
	public class GraphQLReviewsService : IGraphQLReviewsService
	{
		...
		public GraphQLReviewsService(IMemoryCache memoryCache, IConfiguration configuration, IGraphQLProvider graphQLProvider)
		{
		...
		public SearchParams CreateSearchParams()
		{
			...
			{
				FacetOn = new List<string>() { "" }
			};

		}


		public async Task<SubmissionsSearchResults> Search(SearchParams searchParams)
		{
			
			return await SubmissionsSearchResults(searchParams, true);

		}

		async Task<SubmissionsSearchResults> SubmissionsSearchResults(SearchParams searchParams, bool correctFacets)
		{
			...
			fieldFilters.Add(new FieldFilter() { name = "_templatename", value = "Application" });
			fieldsEqualsList.AddRange(fieldFilters);
			GraphQL.GraphQLResponse<Response> response = await _graphQLProvider.SendQueryAsync<Response>(searchParams.IsInEditingMode, GraphQLFiles.ApplicationsSearchAdvanced, new
			{
			...
			var result = new SubmissionsSearchResults
			{
				Application = response.Data.Search.results.items.Select(x => x.item),
		...
		async Task<SubmissionsSearchResults> CorrectFacetCounts(SearchParams searchParams, SubmissionsSearchResults originalResults)
		{
			foreach (string facetOn in searchParams.FacetOn)
			{
				...
				var searchResultsTemp = await SubmissionsSearchResults(temp, false);
